...
void matrix_vector_multiply_mpi(double **matrix, double *vector, double *result, int rows, int cols, int rank, int size) {
    ...
    // Perform partial matrix-vector multiplication
...
int main(int argc, char *argv[]) {
    ...
    matrix_vector_multiply_mpi(matrix, vector, result, rows, cols, rank, size);
    
    // Display result
    if (rank == 0) {
        printf("Resultant vector after matrix-vector multiplication using MPI:\n");
        for (int i = 0; i < rows; i++) {
            printf("%lf ", result[i]);
        }
        printf("\n");
