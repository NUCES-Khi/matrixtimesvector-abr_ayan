...
void matrix_vector_multiply_openmp(double **matrix, double *vector, double *result, int rows, int cols) {
...
int main(int argc, char *argv[]) {
    ...
    matrix_vector_multiply_openmp(matrix, vector, result, rows, cols);
    
    // Display result
    printf("Resultant vector after matrix-vector multiplication using OpenMP:\n");
    for (int i = 0; i < rows; i++) {
        printf("%lf ", result[i]);
    }
    printf("\n");
